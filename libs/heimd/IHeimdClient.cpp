...
class BpHeimdClient : public BpInterface<IHeimdClient>
{
public:
    BpHeimdClient(const sp<IBinder>& impl)
	: BpInterface<IHeimdClient>(impl)
    {
    }

    void State(WifiState state) {
	Parcel data, reply;
	data.writeInterfaceToken(IHeimdClient::getInterfaceDescriptor());
...
status_t BnHeimdClient::onTransact( uint32_t code, 
				    const Parcel& data, 
				    Parcel* reply, 
				    uint32_t flags )
{
    switch(code) {
    case STATE: {
	CHECK_INTERFACE(IHeimdClient, data, reply);
...
IMPLEMENT_META_INTERFACE(HeimdClient, "HeimdClient")
