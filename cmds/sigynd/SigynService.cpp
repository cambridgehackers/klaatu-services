...
public:
    ...
    static RILRequest *create(int token, int message, const sp<ISigynClient>& client) {
	return new RILRequest(client, token, message);
    }

    // Used internally by SigynService
    ...
    const sp<ISigynClient> client() const { return mClient; }
...
private:
    RILRequest(const sp<ISigynClient>& client, int token, int message) 
	: mClient(client)
	, mToken(token)
	, mMessage(message)
...
{
public:
    SigynServerClient(const sp<android::ISigynClient>& c, UnsolicitedMessages f)
	: client(c), flags(f) {}

    sp<android::ISigynClient> client;
...
{
    SigynService *service = (SigynService *)cookie;
    return service->incomingThread();
}

int SigynService::beginOutgoingThread(void *cookie)
{
    SigynService *service = (SigynService *)cookie;
...
int SigynService::incomingThread()
{
    while (1) {
	uint32_t header;
	...
	if (ret != data_size) {
	    perror("SigynService::incomingThread read on payload");
	    return ret;
	}
...
int SigynService::outgoingThread()
{
...
void SigynService::sendToRILD(RILRequest *request)
{
    Mutex::Autolock _l(mLock);
...
RILRequest *SigynService::getPending(int serial_number)
{
    Mutex::Autolock _l(mLock);
...
void SigynService::receiveUnsolicited(const Parcel& data)
{
	...
	for (size_t i = 0 ; i < mClients.size() ; i++) {
	    SigynServerClient *client = mClients.valueAt(i);
...
void SigynService::receiveSolicited(const Parcel& data)
{
...
/*
  Lookup the SigynServerClient.  If it doesn't exist, create a new empty one.
  You must hold the mLock before calling this method
 */

SigynServerClient * SigynService::getClient(const sp<ISigynClient>& client)
{
...
status_t BnSigynService::onTransact( uint32_t code, 
				    const Parcel& data, 
				    Parcel* reply, 
				    uint32_t flags )
{
    switch(code) {
    case REGISTER: {
	CHECK_INTERFACE(ISigynSession, data, reply);
	sp<ISigynClient> client = interface_cast<ISigynClient>(data.readStrongBinder());
...
ANDROID_SINGLETON_STATIC_INSTANCE(SigynService)
