...
/* Begin functions called from a wifi thread */
...
/* End functions called from a wifi thread */
...
void WifiStateMachine::handleSupplicantConnection()
{
	...
	WifiService::getInstance().BroadcastInformation(mWifiInformation);
    }
    Mutex::Autolock _l(mReadLock);
    // ### TODO: This can stall.  Not a good idea
    mWifiConfigStore->initialize();
    WifiService::getInstance().BroadcastConfiguredStations(mWifiConfigStore->stationList());
}
...
void WifiStateMachine::handleScanData(const String8& data)
{
    ...
    mScanResultIsPending = false;
    WifiService::getInstance().BroadcastScanResults(mScannedStations->data());
}
...
void WifiStateMachine::fetchRssiAndLinkSpeedNative()
{
    ...
    WifiService::getInstance().BroadcastRssi(mWifiInformation.rssi);

    if (link_speed != -1) {
	mWifiInformation.link_speed = link_speed;
	WifiService::getInstance().BroadcastLinkSpeed(link_speed);
    }
...
void WifiStateMachine::setState(WifiState state)
{
    ...
    if (state != mState) {
	mState = state;
	WifiService::getInstance().BroadcastState(state);
    }
}
